...
public class Player {
    ...
    // method called by SummonMoves to add a minion which only gets added at the end of the round
    ...
    // method to create a new empowered attack element if player has used an empower move
    ...
    public void printCharges() {
        System.out.println(this.name + " has " + charges.get(Move.ChargeType.FIRE) + " fire charges, " + charges.get(Move.ChargeType.AIR) + " air charges, " + charges.get(Move.ChargeType.WATER) + " water charges, " + charges.get(Move.ChargeType.DARK) + " dark charges, " + charges.get(Move.ChargeType.EARTH) + " earth charges.");
    }
    ...
    public void printMinions() {
        for (Creature x:currentMinions) {
            System.out.println(this.name + " has a " + x.getCreatureName() + " with " + x.getCreatureHP() + " health remaining, which attacks for " + x.getAttackElement().getDamage() + " " + x.getAttackElement().getType() + " damage.");
        }
    }

    // method to add elements of EnumMaps, used for charge calculations
    private EnumMap<Move.ChargeType, Integer> addEnumMaps(EnumMap<Move.ChargeType, Integer> charges1, EnumMap<Move.ChargeType, Integer> charges2) {
    ...
    public ArrayList<Integer> getPlayerActions(GameLogic game, Scanner scanner, ArrayList<Move> moves) {
        ...
        // checking that final action is not the second hand of a magic arrow before taking input
        if (i < maxActions) {
    ...
    // if attacks are unblockable, add damage, otherwise move to new list and then update attacks after
    public int unblockableDamageCalculations() {
        ...
        this.attacks.addAll(_attackCopy);
        _attackCopy.removeAll(_attackCopy);
        return _damage;
    }

    // method to update player charges at the end of the round and reset tempCharges and costOfMoves 
