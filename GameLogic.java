...
public class GameLogic {
    ...
    public GameLogic() {
        ...
        moves.add(new AttackMove(1, "unblockable", unblockableAttackCost, "You struck the enemy with an unblockable attack!"));
        moves.add(new SummonMove(1, "golem", 1, "attack", summonGolemCost, "You summoned a golem!"));
        moves.add(new AttackAndBlockMove(2, "strike", 1, "normalBlock", magmaStrikeCost, "You attempted to attack the enemy and block!"));
    ...
    private void executeAllMoves(Player player1, Player player2, ArrayList<Integer> actions1, ArrayList<Integer> actions2) {
    ...
    // method to sort attacks by damage in descending order, with typeblocked attacks at the end of the array list
    private ArrayList<AttackElement> sortAttacks(ArrayList<AttackElement> attacks, boolean[] typeBlocks) {
        ...
        // unblockable attacks moved to tempAttacks last, since these can never be blocked or countered
        for (int i = 0; i < attacks.size(); i++) {
        ...
        // update attack lists so that typeblocked attack and unblockable attacks are at the end
    ...
    private ArrayList<AttackElement> allCounters(ArrayList<AttackElement> attacks, ArrayList<CounterElement> counters, Player attackingPlayer) {
        ...
        // if statement used to ensure attacks are only set to unblocked attacks if countering occurred
        if (isCountering) {
            while (isCountering == true) {
                // if attack is countered, add it to counterattacks and remove from attacks and counters
                if (counters.get(0).counterCheck(attacks.get(0))) {
                    ...
                    // if attack is not countered, add to unblocked attacks and remove from counter considerations
    ...
    private void minionDeathCheck(ArrayList<AttackElement> attacks1, ArrayList<Creature> minions2, Player attacker, Player minionmancer) {
        boolean _minionCombat = true;
        ...
        while (_minionCombat) {            
            // each instance deals with the damage for a single attack and we keep going until all minions dead or all attacks dealt with
    ...
    // method to calculate damage to minions and remove from their HP for a single attack
    private void minionCombat(ArrayList<AttackElement> attacks1, ArrayList<Creature> minions2, Player player1, Player player2) {
    ...
    public void removeTypeblockedAttacks(ArrayList<AttackElement> attacks1, boolean[] typeBlocks2, String type, int index) {
        // new array list used to prevent NoSuchElementExceptions from changing a list mid loop
    ...
    public void blockAttackIndex(ArrayList<AttackElement> attacks1, ArrayList<BlockElement> blocks2, int index) {
        ...
        // plusblock is used to add up block value of blocks which weren't sufficient to block an attack on their own
        int plusBlock = 0;
        ...
        ArrayList<AttackElement> _attackCopy = new ArrayList<AttackElement>();
        _attackCopy.addAll(attacks1);
        ArrayList<BlockElement> _blockCopy = new ArrayList<BlockElement>();
        _blockCopy.addAll(blocks2);

        while (_isBlocking) {
            // if block is successful, reset plusblock and remove attack and blocks (could be multiple blocks due to plusBlock)
            if (_blockCopy.get(0).blockCheck(_attackCopy.get(index), plusBlock)) {
                plusBlock = 0;
                _attackCopy.remove(index);
                _blockCopy.remove(0);

                // update attacks and blocks after successful block
                attacks1.remove(index);
                blocks2.removeAll(blocks2);
                blocks2.addAll(_blockCopy);
            }
            else {
                // add block value to plusBlock and remove from _blockCopy to check whether a combination of blocks can block the attack
                plusBlock = plusBlock + _blockCopy.get(0).getBlock();
                _blockCopy.remove(0);
            }

            // if we are done blocking, stop
            if (_blockCopy.size() == 0 || _attackCopy.size() == 0) {
                _isBlocking = false;
                plusBlock = 0;
            }
        }
    ...
    private void blockCalculations(ArrayList<AttackElement> attacks1, ArrayList<BlockElement> blocks2, boolean[] typeBlocks2) {
    ...
    private int[] calculateDamage(Player player1, Player player2) {
        ...
        // check for minion deaths as minions can't block attacks and will bodyblock for player
        ...
        // remove unblockable attacks and add to player damage now that minions have been killed
    ...
    public void turn(Player player1, Player player2, GameLogic game, Scanner scanner) {
        ...
        System.out.println("\n" + player1.getName() + " took " + damage[1] + " damage and has " + Math.max(player1.getHP(), 0) + " health remaining.");
        System.out.println(player2.getName() + " took " + damage[0] + " damage and has " + Math.max(player2.getHP(), 0) + " health remaining.");
